...
(function (root) {
  ...
  const types = {
    1: {name:'המתקן', theme:'לעשות את הדבר הנכון', color:'#a95829', description:'הדפוס הזה מחפש יושרה, דיוק ואחריות. הצורך לפעול כראוי עשוי לעזור לשפר דברים, ולפעמים להפוך לביקורת פנימית שקשה להשקיט.', strength:'יכולת להבחין במה שדורש תיקון ולהשקיע בו בהתמדה.', cost:'המתח בין המצוי לרצוי עלול להשאיר מעט מקום למנוחה ולחמלה.', practice:'לבחור היום משימה אחת ולהחליט מראש מה ייחשב בה טוב מספיק.', reflection:'אם לא הייתי צריך להצדיק את עצמי, מה הייתי מרשה לעצמי עכשיו?'},
    2: {name:'המעניק', theme:'להרגיש אהוב וחשוב', color:'#a34356', description:'הדפוס הזה מחפש קרבה דרך נתינה ותשומת לב לאחרים. לפעמים הצורך להיות נחוץ מקשה לזהות צורך אישי או לבקש משהו במפורש.', strength:'יכולת לזהות מצוקה, להציע עזרה וליצור חום בקשר.', cost:'נתינה ללא בקשה ברורה עלולה להתחלף באכזבה כשלא מקבלים הכרה.', practice:'לבקש היום דבר קטן ישירות, בלי להקדים לו שירות או טובה.', reflection:'מה אני צריך גם אם אף אחד לא צריך ממני דבר?'},
    3: {name:'המקדם', theme:'להרגיש בעל ערך דרך עשייה', color:'#946021', description:'הדפוס הזה מכוון לתוצאות, להתקדמות ולהכרה. הוא יודע להתאים את עצמו למשימה, אך עשוי לקשור את תחושת הערך לאופן שבו ההישגים נראים מבחוץ.', strength:'יכולת להפוך מטרה לעשייה ולהניע תהליך קדימה.', cost:'המרדף אחר התוצאה עלול לדחוק הצידה עייפות, רגשות ורצונות אישיים.', practice:'להקדיש זמן קצר לדבר בעל ערך עבורך שאין לו תוצאה שאפשר להציג.', reflection:'מה הייתי בוחר אילו איש לא היה יודע שהצלחתי?'},
    4: {name:'המחפש הייחודי', theme:'לחיות מתוך זהות ומשמעות', color:'#7b4b9c', description:'הדפוס הזה מקשיב לעולם הפנימי ומחפש ביטוי אישי אותנטי. לעיתים תשומת הלב נמשכת למה שחסר, עד שקשה לחוש את מה שכבר נוכח.', strength:'רגישות לגוונים רגשיים ויכולת לתת לחוויה משמעות אישית.', cost:'השוואה לאידיאל או לאחרים עלולה להעמיק תחושת חוסר ושונות.', practice:'לתת שם לרגש הנוכחי, ואז לעשות פעולה קטנה גם בלי לחכות למצב הרוח המתאים.', reflection:'מה בחיים שלי ראוי לתשומת לב גם כשהוא רגיל ופשוט?'},
    5: {name:'החוקר', theme:'להבין ולהיות מסוגל', color:'#3c6195', description:'הדפוס הזה מחפש הבנה, עצמאות ומרחב לעיבוד. כשהדרישות נחוות כמציפות, הוא עשוי להגן על הזמן והאנרגיה באמצעות התרחקות.', strength:'יכולת להעמיק, לזהות מבנים ולחשוב באופן עצמאי.', cost:'ההמתנה למספיק ידע או אנרגיה עלולה לדחות השתתפות והתנסות.', practice:'לשתף רעיון שעדיין אינו מושלם, או להשתתף בצעד קטן לפני שהכול ברור.', reflection:'מה אפשר ללמוד דווקא מתוך מעורבות ולא רק מתוך התבוננות?'},
    6: {name:'הנאמן', theme:'למצוא על מה אפשר לסמוך', color:'#287a7c', description:'הדפוס הזה בוחן אמינות, אחריות והיערכות. החיפוש אחר ביטחון עשוי להתבטא בבדיקות ובבקשת תמיכה, ולעיתים דווקא בעימות ישיר עם החשש.', strength:'יכולת לצפות קשיים, לשאול שאלות טובות ולעמוד לצד אחרים.', cost:'בדיקה נוספת ועוד אחת עלולות להרחיק מתחושת הכרעה.', practice:'בהחלטה קטנה, להגדיר מראש כמה מידע מספיק ואז לבחור.', reflection:'איזה חלק מהמצב דורש עוד מידע, ואיזה חלק דורש אמון ביכולת שלי להתמודד?'},
    7: {name:'המגלה', theme:'להשאיר מקום לאפשרויות', color:'#907022', description:'הדפוס הזה מחפש חופש, חיות ואפשרויות. הוא עשוי להתרחק מכאב או מתחושת הגבלה באמצעות תוכניות, רעיונות וחוויות חדשות.', strength:'יכולת לראות אפשרויות, לאלתר ולהביא התלהבות.', cost:'המעבר לדבר הבא עלול לבוא על חשבון עומק, השלמה ומפגש עם קושי.', practice:'להישאר עוד כמה דקות במשימה אחת או ברגש לא נעים, בלי לפתוח חלופה חדשה.', reflection:'מה אמצא כאן אם לא אמהר לעבור הלאה?'},
    8: {name:'המגן', theme:'לשמור על עצמאות והשפעה', color:'#a14739', description:'הדפוס הזה רגיש לשליטה מבחוץ ומחפש יכולת לפעול ולהגן. הוא נוטה לפגוש קושי ישירות, אך עשוי להתקשות לחשוף צורך או פגיעות.', strength:'יכולת לתפוס עמדה, להציב גבול ולהגן על מה שחשוב.', cost:'עוצמה וישירות עלולות להשאיר פחות מקום להקשבה ולסיוע מאחרים.', practice:'לבטא צורך אחד בלי להסתיר אותו מאחורי דרישה, ולהקשיב לפני תגובה.', reflection:'איפה אפשר להישען על מישהו בלי לוותר על העצמאות שלי?'},
    9: {name:'המגשר', theme:'לשמור על שקט וחיבור', color:'#4c7955', description:'הדפוס הזה מחפש הרמוניה ורואה כמה נקודות מבט. לפעמים הוא שומר על השקט באמצעות דחיית רצונותיו או הימנעות ממתח שדורש פעולה.', strength:'יכולת להקשיב, להכיל מורכבות וליצור מרחב נעים.', cost:'ההסתגלות לסביבה עלולה לטשטש סדרי עדיפויות ורצון אישי.', practice:'לבחור היום העדפה קטנה ולומר אותה בקול לפני שמבררים מה כולם רוצים.', reflection:'מה חשוב לי כאן, גם אם מישהו אחר יעדיף אחרת?'}
  };
  const instincts = {
  ...
  function getWing(type, scores, primaryClear=true) {
    ...
    const status = !primaryClear ? 'conditional' : ranked[0].score < 40 ? 'weak' : gap < WING_GAP ? 'close' : 'suggested';
    return {adjacent,ranked,gap,status,dominant:status==='suggested'?Number(ranked[0].key):null};
  }
  function score(answers) {
    ...
    const counts={...totals};
    questions.forEach((q,i)=>{totals[q.scale]+=q.reverse?6-answers[i]:answers[i]; counts[q.scale]++;});
    ...
    const primaryStatus=uniform?'undifferentiated':typeRanking[0].score<50?'weak':gap<TYPE_GAP?'close':'suggested';
    const primary=primaryStatus==='suggested'?Number(typeRanking[0].key):null;
    const candidates=typeRanking.filter(t=>typeRanking[0].score-t.score<TYPE_GAP).map(t=>Number(t.key));
    ...
    return {version:VERSION,scores,typeRanking,instinctRanking,gap,primaryStatus,primary,candidates,
  ...
  const api={VERSION,TYPE_GAP,WING_GAP,INSTINCT_GAP,questions,types,instincts,wingNotes,score,getWing,validState};
