(function () {
  ...
  function getStoredState(key){const value=read(key);if(value===null)return null;if(E.validState(value))return value;const migrated=E.migrateState(value);if(migrated){write(key,migrated);notice(value.version==='2.0.0'?'התשובות הקודמות נשמרו. נוספו 18 שאלות, ויש להשלים אותן לפני חישוב תוצאה מעודכנת.':'התוצאה עודכנה לפי התשובות השמורות שלך.');return migrated;}remove(key);notice('נמצאה שמירה ישנה או לא תקינה שלא ניתן לשחזר. אפשר להתחיל מבחן חדש.');return null;}
  ...
  function wingTitle(r){
    ...
    if(r.wing.dominant)return`כנף מובילה: ${r.primary}w${r.wing.dominant}`;
  ...
  function reportData(){
    return {schema:'nh-enneagram-result',version:E.VERSION,createdAt:completedAt,questionCount:E.questions.length,
      ...
      scoring:{selection:'style-ranking-with-adjacent-comparisons',coreWindow:E.CORE_WINDOW,pairMinimum:E.PAIR_MIN,requireLeaveOneOutStability:false,wingGap:E.WING_GAP,wingMinimum:E.WING_MIN,wingRatio:E.WING_RATIO,instinctGap:E.INSTINCT_GAP},
  ...
  function reportHtml(){
    ...
    const limitations='הציונים בסולם 0 עד 100 מתארים התאמה לתשובות, ולא הסתברות או אחוזי ודאות. הספים הם כללי תצוגה של הכלי, לא ערכים מתוקפים מחקרית. ציוני הסגנונות הם ממוצעי שש האמירות לכל טיפוס. 18 ההשוואות מחושבות בנפרד. מתחילים מהדירוג המקורי ובוחנים מועמדים עד 15 נקודות מהמוביל. שתי העדפות עקביות של שכן מועמד, בממוצע של 75 לפחות, מסירות את הטיפוס שהפסיד מהבחירה הסופית. בוחרים את הציון המקורי הגבוה מבין הנותרים. בהיעדר הכרעה או במעגל השוואות סותר חוזרים למוביל המקורי. בתיקו מלא בציונים ובמידע ההשוואתי הסדר מספרי. כנף דורשת ציון של 62.5 לפחות ולפחות 75% מציון הטיפוס. מבין השכנים שעומדים בסף, בעל הציון הגבוה מוצג כמוביל, גם בפער קטן. בתיקו בין השכנים אין כנף מובילה. מענה אחיד אינו מוכרע. סדר מלא של אינסטינקטים דורש פער של 8 נקודות גם בין השני לשלישי. השאלון לא עבר תיקוף על מדגם אנושי. סקירת המחקר מצאה ראיות מעורבות לתוקף האניאגרמה ומעט תמיכה בכנפיים. זה אינו שאלון רשמי של Enneagram Institute.';
