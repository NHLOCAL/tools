(function () {
  ...
  function showType(n){
    ...
    $('type-detail').innerHTML=`<h3>${n} · ${data.name}: ${data.theme}</h3><p>${data.description}</p><div class="detail-grid"><div><h3>מה הדפוס מאפשר</h3><p>${data.strength}</p></div><div><h3>מה עלול לגבות מחיר</h3><p>${data.cost}</p></div></div><div class="practice"><h3>ניסוי קטן לשבוע הקרוב</h3><p>${data.practice}</p><p class="reflection">${data.reflection}</p></div><details style="margin-top:20px"><summary>שתי הכנפיים של טיפוס ${n}</summary>${neighbors.map(w=>`<p><strong><bdi>${n}w${w}</bdi>:</strong> ${E.wingNotes[`${n}w${w}`]}</p>`).join('')}</details>`;
  }
  ...
  function reportHtml(){
    ...
    const descriptions=r.typeRanking.slice(0,3).map(({key})=>{const t=E.types[key];return`<section><h3>טיפוס ${key} · ${t.name}</h3><p>${t.description}</p><p><strong>משאב:</strong> ${t.strength}</p><p><strong>אתגר:</strong> ${t.cost}</p><p><strong>ניסוי קטן:</strong> ${t.practice}</p><p><strong>שאלה למחשבה:</strong> ${t.reflection}</p></section>`;}).join('');
